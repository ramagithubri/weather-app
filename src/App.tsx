...
// Using Nominatim (OpenStreetMap) for better geocoding of small towns and villages
...
function App() {
  ...
  const fetchWeather = async (query: string) => {
    ...
    try {
      // Nominatim prefers natural language queries or comma separated
      const geoRes = await fetch(`${GEOCODING_API}?q=${encodeURIComponent(query)}&format=json&limit=1&accept-language=en`, {
        headers: {
      ...
      const weatherRes = await fetch(
        `${WEATHER_API}?latitude=${location.lat}&longitude=${location.lon}&current=temperature_2m,relative_humidity_2m,apparent_temperature,is_day,precipitation,weather_code,wind_speed_10m&daily=temperature_2m_max,temperature_2m_min&timezone=auto`
      );
